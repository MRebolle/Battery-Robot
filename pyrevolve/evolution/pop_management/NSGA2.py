...
class NSGA2:
    ...
    def __call__(self, selection_pool, conf, gen_num):
        population_size = conf.population_size
        offspring_size = conf.offspring_size

        final_season = list(conf.environments.keys())[-1]


        # Preparate the objectives as a matrix of individuals in the rows and fitnesses in the columns.
        objectives = np.zeros((population_size + offspring_size, max(1, len(selection_pool[0][final_season].objectives))))  # TODO fitnesses is 0

        # Fill the objectives with all individual from the population and offspring combined.
        all_individuals = copy.deepcopy(selection_pool)
        print("creating objectives")
        print(f'Population size is {population_size}, and offspring size is {offspring_size}')
        for index, individual in enumerate(all_individuals):
            ...
            # TODO this could not work when testing multiple environments. Test
            objectives[index, :] = [-objective for objective in individual[final_season].objectives]
        ...
        if self.debug:
           self._visualize(objectives, sorted_fronts, new_individuals, discarded_individuals, front_no, final_season, gen_num)

        return new_individuals

    def _visualize(self, objectives, sorted_fronts, new_individuals, discarded_population, front_no, environment, gen_num):
        ...
        for individual in new_individuals:
            ax.scatter(individual[environment].objectives[0], individual[environment].objectives[1], s=3, color='black')

        for individual in discarded_population:
            ax.scatter(individual[environment].objectives[0], individual[environment].objectives[1], s=2, color='white')

        nsga_generation_plot_path = self.experiment_management.plot_path(data_source="NSGA2", filename='nsga2_front_', generation=gen_num)
