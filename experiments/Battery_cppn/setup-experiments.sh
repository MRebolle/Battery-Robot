...
experiments_path=Battery_cppn/Data/
managers_path=experiments/Battery_cppn/
...
while true
    ...
    do
         ...
         echo ""
         screen -d -m -S "${experiment}" -L -Logfile "${experiment}.log" nice -n19 ./revolve.sh --manager "${managers_path}$(cut -d'_' -f1 <<<"$experiment").py" --experiment-name "${experiments_path}${experiment}" --n-cores 4 --port-start $start_port
